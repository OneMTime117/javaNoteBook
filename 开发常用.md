...
- get（ChronoField）：通过枚举，获取指定形式的时间数据，返回int类型

- getLong（ChronoField）：和get（ChronoField）一致，用于查询数据类型为long的（可以通过枚举的value范围进行判断）
...
两者本质上都是通过ClassLoader加载资源，和获取URL类似，只是获取的为当前资源的输入流，但该方式可以**有效避免使用文件路径无法获取压缩包下的资源**，通过ClassLoader可以直接通过URL来定位获取资源
