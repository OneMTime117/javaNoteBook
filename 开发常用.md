...
- Pattern
  ...
  除了提供实例化静态方法来创建正则表达式模式实例外，Pattern还提供matches方法，只进行一次正则表达式匹配：
...
​		用于简化**创建只有一个抽象方法接口（函数式接口）的实例**  的代码，实现函数式编程
...
​		Optional主要用于，对某个对象实例进行封装，当再次使用时，以链式方式调用，无需进行if-null判断，并能构建流畅的API，比如streamAPI中findFirest()方法：
...
​		Regular expression（正规表达式），缩写为**regex/regexp**；用于通过一定的匹配模型，实现字符串替换、提取和校验
...
​		在匹配成功后，会对匹配成功的字符串前后进行断言判断，如果失败，就舍弃该匹配成功的字符串；即，额外对匹配字段添加了约束，保证其前后字符满足另外一个匹配条件

- ？=  先行断言-满足

  当匹配第一部分表达式后，对其后面内容进行断言判断，满足，则保留匹配成功的字符串

  the（？=fa），对于匹配the的字符串，其后面必须为fa

- ？！先行断言-不满足

  当匹配第一部分表达式后，对其后面内容进行断言判断，满足，则保留匹配成功的字符串

  the（？！fa），对于匹配the的字符串，其后面不能为fa

- ？<= 后发断言-满足

  当匹配第二部分表达式后，对其前面内容进行断言判断，满足，则保留匹配成功的字符串

  （？<=the）fat，对于匹配fat的字符串，其前面必须为the

- ？<! 后发断言-不满足

  当匹配第二部分表达式后，对其前面内容进行断言判断，不满足，则保留匹配成功的字符串

  （？<!the）fat,对于匹配fat的字符串，其前面不能为the

### 5、标志

​		标志也叫模式修正符，用于改变正则表达式的匹配模式

通过/ /来包裹正则表达式，然后在后面添加模式参数：
...
| g    | 全局搜索（Global search），返回所有匹配结果                  |
| m    | 多行匹配（Multiline），当使用锚点进行字符串开头和结尾匹配时，可以实现在每行都生效 |

### 6、贪婪匹配和惰性匹配

正则表达式默认使用贪婪匹配，即对于重复次数匹配会尽可能多，此时我们可以通过额外添加？元字符， 将贪婪匹配模式转化为惰性匹配模式

```
is the food the good
.*the   匹配 is the food the   
.*?the  匹配 is the
```

## 转义字符

​		转义字符是很多语言、程序、数据格式的形式文本的一部分，方便表示一些特殊字符进行，不同语言环境下，转义字符有所不同：
...
对于普通字符，如果在语言中有其他含义，则需要添加 \转义字符，来表示它的原意，如
